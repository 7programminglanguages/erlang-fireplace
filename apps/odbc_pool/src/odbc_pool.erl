...
%%% Purpose :
%%% Created :  25 Nov 2018 by XiangKui <xiangkui@bmxlabs.com>
%%%-------------------------------------------------------------------------------------------------
%%%
...
-define(STATE_KEY, odbc_pool_state).

-define(NESTING_KEY, odbc_pool_nesting_level).
...
start(Host) ->
    p1_fsm:start(odbc_pool, [Host],
                     fsm_limit_opts() ++ (?FSMOPTS)).

start_link(Config) ->
    p1_fsm:start_link(odbc_pool,
                          Config,
                          fsm_limit_opts() ++ (?FSMOPTS)).
...
init(Config) ->
    ...
    {ok, connecting,
     #state{db_type = mysql, db_host_opts = DBHostOpts,
...
sql_call(Host, Msg) ->
    ...
    case SqlResult of
        {error, timeout} ->
            maybe_add_metrics(Host, timeout, 1);
        {error, "query timed out"} ->
            maybe_add_metrics(Host, query_timed_out, 1);
        Other ->
            maybe_add_metrics(Host, element(1, Other), 1)
    end,
    SqlResult.
...
sql_call_1(Host, Msg) ->
    case get(?STATE_KEY) of
        undefined ->
            case odbc_pool_sup:get_random_pid(Host) of
                undefined ->
...
max_queue() ->
    application:get_env(odbc_pool, max_queue, 1000).
...
maybe_add_metrics(Service, Result, Num) ->
    case application:get_env(odbc_pool, metrics_module, undefined) of
        undefined ->
            skip;
        Module ->
            Module:inc(odbc, [Service, Result], Num)
    end.
