%%%-------------------------------------------------------------------------------------------------
%%% File    :  xsync_message_history_worker.erl
...
%%%
%%%-------------------------------------------------------------------------------------------------
-module(xsync_message_history_worker).
        ...
        {ok, Value, Ack} ->
            case parse_msg(Value) of
                {error, _} ->
                    ...
                    {State,0};
                {ok, KafkaGroupNotice} ->
                    {ok, ShaperValue} = send_msg(KafkaGroupNotice),
                    ack_msg(KafkaService, Ack),
...
parse_msg(Value) ->
    try kafka_message_history_pb:decode_msg(Value, 'KafkaMessageHistory') of
        #'KafkaMessageHistory'{} = KafkaMessageHistory ->
            {ok, KafkaMessageHistory}
    catch
        C:E ->
...
send_msg(#'KafkaMessageHistory'{} = KafkaMessageHistory) ->
    spawn(fun() ->
                  xsync_message_history_handler:handle(KafkaMessageHistory)
          end),
    {ok, 1}.
