%%%-------------------------------------------------------------------------------------------------
%%% File    :  xsync_group_notice_worker.erl
...
%%%
%%%-------------------------------------------------------------------------------------------------
-module(xsync_group_notice_worker).
...
parse_msg(Value) ->
    try kafka_notice_pb:decode_msg(Value, 'KafkaGroupNotice') of
        #'KafkaGroupNotice'{} = KafkaGroupNotice ->
            {ok, KafkaGroupNotice}
    catch
        C:E ->
...
send_msg(#'KafkaGroupNotice'{} = GroupNotice) ->
    spawn(fun() ->
                  xsync_group_notice_handler:handle(GroupNotice)
          end),
    {ok, 1}.
