%%%-------------------------------------------------------------------------------------------------
%%% File    :  xsync_user_notice_worker.erl
...
%%%
%%%-------------------------------------------------------------------------------------------------
-module(xsync_user_notice_worker).
...
parse_msg(Value) ->
    try kafka_notice_pb:decode_msg(Value, 'KafkaUserNotice') of
        #'KafkaUserNotice'{} = KafkaUserNotice ->
            {ok, KafkaUserNotice}
    catch
        C:E ->
...
send_msg(#'KafkaUserNotice'{} = RosterNotice) ->
    spawn(fun() ->
                  xsync_user_notice_handler:handle(RosterNotice)
          end),
    {ok, 1}.
